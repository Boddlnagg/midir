...
impl MidiInputPort {
    ...
    fn interface_id(port_number: UINT) -> Result<Box<[u16]>, PortInfoError> {
        ...
        let result = unsafe {
            winapi::um::mmeapi::midiInMessage(
                port_number as HMIDIIN,
        ...
        unsafe {
            let result = winapi::um::mmeapi::midiInMessage(
                port_number as HMIDIIN,
    ...
    fn name(port_number: UINT) -> Result<String, PortInfoError> {
        ...
        let output = from_wide_ptr(pname.as_ptr(), pname.len())
            .to_string_lossy()
            .into_owned();
        Ok(output)
    }
        ...
        F: FnMut(u64, &[u8], &mut T) + Send + 'static,
    {
        ...
        for i in 0..RT_SYSEX_BUFFER_COUNT {
            handler_data.sysex_buffer.0[i] = Box::into_raw(Box::new(MIDIHDR {
                lpData: unsafe {
                    allocate(RT_SYSEX_BUFFER_SIZE /*, mem::align_of::<u8>()*/)
                } as *mut i8,
    ...
    fn close_internal(&mut self) {
        ...
        for i in 0..RT_SYSEX_BUFFER_COUNT {
            ...
            unsafe {
                ...
                deallocate(
                    (*self.handler_data.sysex_buffer.0[i]).lpData as *mut u8,
                    RT_SYSEX_BUFFER_SIZE, /*, mem::align_of::<u8>()*/
                );
...
impl MidiOutputPort {
    ...
    fn interface_id(port_number: UINT) -> Result<Box<[u16]>, PortInfoError> {
        let mut buffer_size: winapi::shared::minwindef::ULONG = 0;
        let result = unsafe {
            winapi::um::mmeapi::midiOutMessage(
                port_number as HMIDIOUT,
                DRV_QUERYDEVICEINTERFACESIZE,
                &mut buffer_size as *mut _ as DWORD_PTR,
                0,
            )
        ...
        let mut buffer = Vec::<u16>::with_capacity(buffer_size as usize / 2);
        unsafe {
            let result = winapi::um::mmeapi::midiOutMessage(
                port_number as HMIDIOUT,
                DRV_QUERYDEVICEINTERFACE,
                buffer.as_mut_ptr() as DWORD_PTR,
                buffer_size as DWORD_PTR,
