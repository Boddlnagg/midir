...
impl MidiInput {
    ...
    pub(crate) fn ports_internal(&self) -> Vec<::common::MidiInputPort> {
        let device_collection = DeviceInformation::find_all_async_aqs_filter(&self.selector)
            .unwrap()
            .get()
    ...
    pub fn port_name(&self, port: &MidiInputPort) -> Result<String, PortInfoError> {
        let device_info_async = DeviceInformation::create_from_id_async(&port.id)
    ...
    fn handle_input<T>(args: &MidiMessageReceivedEventArgs, handler_data: &mut HandlerData<T>) {
        ...
        let membuffer = Buffer::create_memory_buffer_over_ibuffer(&buffer)
            .expect("create_memory_buffer_over_ibuffer failed");
        byte_access = membuffer
            .create_reference()
            .expect("create_reference failed")
            .try_into()
            .unwrap();
        ...
        F: FnMut(u64, &[u8], &mut T) + Send + 'static,
    {
        ...
        let event_token = in_port
            ...
            .expect("add_message_received failed");
...
impl<T> MidiInputConnection<T> {
    pub fn close(self) -> (MidiInput, T) {
        ...
        let device_selector =
            MidiInPort::get_device_selector().expect("get_device_selector failed"); // probably won't ever fail here, because it worked previously
...
impl MidiOutputConnection {
    pub fn close(self) -> MidiOutput {
        ...
        let _ = closable.close();
        let device_selector =
            MidiOutPort::get_device_selector().expect("get_device_selector failed"); // probably won't ever fail here, because it worked previously
        MidiOutput {
            selector: device_selector,
        }
    }
    ...
    pub fn send(&mut self, message: &[u8]) -> Result<(), SendError> {
        ...
        data_writer
            .write_bytes(message)
            .map_err(|_| SendError::Other("write_bytes failed"))?;
        let buffer = data_writer
            .detach_buffer()
            .map_err(|_| SendError::Other("detach_buffer failed"))?;
        self.port
            .send_buffer(&buffer)
            .map_err(|_| SendError::Other("send_buffer failed"))?;
        Ok(())
    }
}
